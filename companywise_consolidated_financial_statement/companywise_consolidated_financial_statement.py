...
def execute(filters=None):
	...
	filters["company"] = frappe.db.get_list('Company', {'is_group':1},['name'])[0]['name']
	print(filters)
	...
	for row in data:
		...
		for comp in frappe.get_all('Company'):
			...
			acc = row['account_name'] + f' - {abbr}' if 'account_name' in row else None
			if acc:
				...
				if not is_group:
					...
					'account_name': acc, 
					'account': acc, 
					'parent_account': row['parent'] if 'parent' in row else None, 
					'indent': row['indent'] if 'indent' in row else 0, 
					'year_start_date': row['year_start_date'] if 'year_start_date' in row else None, 
					'root_type': row['root_type'] if 'root_type' in row else None,
					'year_end_date': row['year_end_date'] if 'year_end_date' in row else None, 
					'currency': row['currency'] if 'currency' in row else None,
					'opening_balance': row['opening_balance'] if 'opening_balance' in row else None, 
					'val':row[comp] if comp in row else 0,
					'has_value': row['has_value'] if 'has_value' in row else False, 
					'total': row['total'] if 'total' in row else 0
					})
				else:
					total_value += row[comp] if comp in row else 0
		print(total_value,row['account_name'] if 'account_name' in row else None)
		if total_value:
			acc = row['account_name'] if 'account_name' in row else None
			final_data.append(
			{
			'account_name': acc, 
			'account': acc, 
	...
	return columns, final_data, None, None, None
